...
function spawnCommand(cmd, args, cwd) {
  const cp2 = import_child_process.spawn(cmd, args, {cwd});
  let res = "";
...
function runCommand(cmd, opts = {}, timeout) {
  ...
  return new Promise((resolve, reject) => {
    ...
    import_child_process.exec(cmd, opts, (err, stdout, stderr) => {
      if (timer)
        clearTimeout(timer);
      if (err) {
...
function getStdout(cmd, opts = {}, timeout) {
  return new Promise((resolve, reject) => {
    ...
    opts.maxBuffer = 5 * 1024 * 1024;
    import_child_process.exec(cmd, opts, (_err, stdout) => {
      if (timer)
        clearTimeout(timer);
      if (stdout) {
        resolve(stdout);
...
function findGitDarwin(onLookup) {
  return new Promise((c, e) => {
    import_child_process.exec("which git", (err, gitPathBuffer) => {
      if (err) {
...
var CommitsTask = class extends import_events.EventEmitter {
  ...
  start(cmd, args, cwd) {
    this.process = import_child_process2.spawn(cmd, args, {cwd});
    ...
    rl.on("line", (line) => {
      if (!line.length)
        return;
      let res = import_coc2.ansiparse(line);
...
var Bcommits = class extends import_coc2.BasicList {
  constructor(nvim, manager) {
    ...
    this.addAction("diff", async (item, context) => {
      ...
      if (!commit)
        return;
      let content = await import_coc2.runCommand(`git --no-pager show --no-color ${commit}:${file}`, {cwd: root});
      if (!content)
        return;
      let lines = content.replace(/\n$/, "").split(/\r?\n/);
      nvim.pauseNotification();
    ...
    import_coc2.events.on("BufEnter", async (bufnr) => {
      ...
      if (!diff)
        return;
      let res = await nvim.eval(`map(getwininfo(), 'get(v:val["variables"], "easygit_diff_origin", 0)')`);
      let idx = res.findIndex((i) => i == bufnr);
      if (idx == -1)
        await nvim.command("diffoff");
...
var CommitsTask2 = class extends import_events2.EventEmitter {
  ...
  start(cmd, args, cwd) {
    this.process = import_child_process3.spawn(cmd, args, {cwd});
    this.process.on("error", (e) => {
      this.emit("error", e.message);
    });
    this.process.stderr.on("data", (chunk) => {
    ...
    rl.on("line", (line) => {
      if (!line.length)
        return;
      let res = import_coc4.ansiparse(line);
      let idx = res.findIndex((o) => o.foreground == "yellow");
      let message = idx == -1 ? null : res[idx + 1].text;
      let item = res.find((o) => o.foreground == "red" && o.text.length > 4);
      let commit = item ? item.text : "";
  ...
  constructor(nvim, manager) {
    ...
    this.addAction("reset", async (item) => {
      ...
      if (staged && tree) {
        ...
        let confirmed = await import_coc6.window.showPrompt(`remove ${relative}?`);
        if (!confirmed)
          return;
    ...
    this.addAction("preview", async (item, context) => {
      ...
      if (tree_symbol != "M" && index_symbol != "M") {
        await this.previewLocation({
          uri: import_coc6.Uri.file(import_path3.default.join(root, relative)).toString(),
          range: {
...
var GitStatus = class {
  constructor(service) {
    ...
    this.characters = {
      changedDecorator: config.get("changedDecorator"),
      conflictedDecorator: config.get("conflictedDecorator"),
...
var DocumentManager = class {
  ...
  loadConfiguration(e) {
    ...
    let obj = {
      ...
      changedSign: {
        ...
        hlGroup: config.get("changedSign.hlGroup", "DiffChange")
      },
      addedSign: {
        text: config.get("addedSign.text", "+"),
        hlGroup: config.get("addedSign.hlGroup", "DiffAdd")
      },
      removedSign: {
        text: config.get("removedSign.text", "_"),
        hlGroup: config.get("removedSign.hlGroup", "DiffDelete")
      },
      topRemovedSign: {
        text: config.get("topRemovedSign.text", "\u203E"),
        hlGroup: config.get("topRemovedSign.hlGroup", "DiffDelete")
      },
      changeRemovedSign: {
  ...
  async nextConflict() {
    let buf = await this.buffer;
    if (buf)
      await buf.nextConflict();
  }
  async prevConflict() {
    let buf = await this.buffer;
    if (buf)
      await buf.prevConflict();
  }
  async keepCurrent() {
  ...
  async conflictKeepPart(part) {
    let buf = await this.buffer;
    if (buf)
      await buf.conflictKeepPart(part);
  }
  async chunkStage() {
    let buf = await this.buffer;
    if (!buf)
...
async function getRealPath(fullpath) {
  let resolved;
  try {
    resolved = await import_util9.promisify(import_fs3.default.realpath)(fullpath, "utf8");
  } catch (e) {
...
var GitBuffer = class {
  ...
  getChunk(line) {
    ...
    return this.diffs.find((ch) => {
      ...
      let end = changeType === ChangeType.Change && added.count > removed.count ? ch.end + added.count - removed.count : ch.end;
  ...
  async diffDocument(force = false) {
    ...
    if (!diffs || diffs.length == 0) {
      ...
      if (!this.config.realtimeGutters && !force)
        return;
      this.updateGutters();
    }
  }
  ...
  async loadBlames() {
    ...
    let indexed = await this.repo.isIndexed(this.relpath);
    if (indexed)
      result = await this.getBlameInfo();
  ...
  async parseConflicts() {
    if (!this.hasConflicts || !this.config.conflict.enabled)
      return;
...
function renderWord(issue, issueFormat) {
  return issueFormat.split(/(%i|%o|%r|%b|%t|%c|%a|%u)/).map((part) => {
    switch (part) {
      case "%i":
        return issue.id;
      case "%o":
        return issue.repo.split("/")[0];
      case "%r":
        return issue.repo.split("/")[1];
      case "%b":
        return issue.body;
      case "%t":
        return issue.title;
      case "%c":
        return issue.createAt;
      case "%a":
...
function addSource(context, resolver) {
  ...
  async function loadGitLabIssues(res, host) {
  ...
  async function loadIssues(root) {
    ...
    if (issueSources) {
      return Array.prototype.concat.apply([], await Promise.all(issueSources.split(",").map((issueSource) => {
        ...
        switch (issueProvider) {
          case "github":
            return loadGitHubIssues(organizationName, repoName, true);
          default:
            return [];
        }
      })));
    ...
    let host = "";
    let hosts = config.get("gitlab.hosts", []);
    hosts.forEach((item) => {
