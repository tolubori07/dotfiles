...
function resolveGlobalNodePath(tracer) {
  ...
  try {
    ...
    const stdout = (0, import_child_process.spawnSync)(npmCommand, ["config", "get", "prefix"], options).stdout;
    if (!stdout) {
      if (tracer) {
...
function resolveGlobalYarnPath(tracer) {
  ...
  try {
    process.on("SIGPIPE", handler);
    const results = (0, import_child_process.spawnSync)(yarnCommand, ["global", "dir", "--json"], options);
...
var PrettierEditService = class {
  constructor(moduleResolver, loggingService, statusBar) {
    ...
    this.handleActiveTextEditorChanged = async (editor) => {
    ...
    this.getSelectors = async (prettierInstance, uri) => {
      const { languages: languages2 } = await Promise.resolve(prettierInstance.getSupportInfo());
  ...
  registerDisposables() {
    ...
    const textEditorChange = import_coc6.window.onDidChangeActiveTextEditor((editor) => {
      this.handleActiveTextEditorChanged(editor);
    });
    this.handleActiveTextEditorChanged(import_coc6.window.activeTextEditor);
    return [
      packageWatcher,
  ...
  getPrettierOptions(fileName, parser, vsCodeConfig, configOptions, extensionFormattingOptions) {
    ...
    const options = __spreadValues(__spreadValues(__spreadValues(__spreadValues({}, fallbackToVSCodeConfig ? vsOpts : {}), {
      filepath: fileName,
      parser
    }), rangeFormattingOptions || {}), configOptions || {});
...
var TemplateService = class {
  ...
  async writeConfigFile(folderPath) {
    ...
    await (0, import_util3.promisify)(import_fs2.default.writeFile)(outputPath, templateSource, { encoding: "utf8" });
