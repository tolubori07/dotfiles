...
var require_america = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors11) {
    return function(letter, i, exploded) {
      if (letter === " ")
        return letter;
      switch (i % 3) {
        case 0:
          return colors11.red(letter);
        case 1:
          return colors11.white(letter);
        case 2:
          return colors11.blue(letter);
      }
    };
  };
});
...
var require_zebra = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors11) {
    return function(letter, i, exploded) {
      return i % 2 === 0 ? letter : colors11.inverse(letter);
    };
  };
});
  ...
  module2["exports"] = function(colors11) {
    ...
    return function(letter, i, exploded) {
      if (letter === " ") {
        ...
        return colors11[rainbowColors[i++ % rainbowColors.length]](letter);
      }
    };
  };
});
...
var require_random = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors11) {
    ...
    return function(letter, i, exploded) {
      return letter === " " ? letter : colors11[available[Math.round(Math.random() * (available.length - 2))]](letter);
    };
  };
});
...
var require_colors = __commonJS((exports2, module2) => {
  var colors11 = {};
  module2["exports"] = colors11;
  colors11.themes = {};
  var util2 = require("util");
  var ansiStyles = colors11.styles = require_styles();
  var defineProps = Object.defineProperties;
  var newLineRegex = new RegExp(/[\r\n]+/g);
  colors11.supportsColor = require_supports_colors().supportsColor;
  if (typeof colors11.enabled === "undefined") {
    colors11.enabled = colors11.supportsColor() !== false;
  }
  colors11.enable = function() {
    colors11.enabled = true;
  };
  colors11.disable = function() {
    colors11.enabled = false;
  };
  colors11.stripColors = colors11.strip = function(str) {
    return ("" + str).replace(/\x1B\[\d+m/g, "");
  };
  var stylize = colors11.stylize = function stylize2(str, style) {
    if (!colors11.enabled) {
      return str + "";
    }
    var styleMap = ansiStyles[style];
    if (!styleMap && style in colors11) {
      return colors11[style](str);
    }
  ...
  var proto = defineProps(function colors12() {
  }, styles2);
  function applyStyle() {
  ...
  colors11.setTheme = function(theme) {
    ...
    for (var style in theme) {
      (function(style2) {
        colors11[style2] = function(str) {
          if (typeof theme[style2] === "object") {
            var out = str;
            for (var i in theme[style2]) {
              out = colors11[theme[style2][i]](out);
            }
            return out;
          }
          return colors11[theme[style2]](str);
        };
      })(style);
    }
  };
  ...
  colors11.trap = require_trap();
  colors11.zalgo = require_zalgo();
  colors11.maps = {};
  colors11.maps.america = require_america()(colors11);
  colors11.maps.zebra = require_zebra()(colors11);
  colors11.maps.rainbow = require_rainbow()(colors11);
  colors11.maps.random = require_random()(colors11);
  for (var map in colors11.maps) {
    (function(map2) {
      colors11[map2] = function(str) {
        return sequencer(colors11.maps[map2], str);
      };
    })(map);
  }
  defineProps(colors11, init());
});

// node_modules/colors/safe.js
var require_safe = __commonJS((exports2, module2) => {
  var colors11 = require_colors();
  module2["exports"] = colors11;
});
...
var BufferList = class extends import_coc.BasicList {
  ...
  async loadItems(context) {
    ...
    const args = isArgs ? await nvim.eval("map(argv(), 'bufnr(v:val)')") : [];
    return content.split(/\n/).reduce((res, line) => {
      ...
      const bufname = ms[3];
      if (isArgs && args.indexOf(bufnr) === -1)
        return res;
      if (isPWD && (bufname[0] === "/" || bufname[0] === "~"))
        return res;
      const item = {
...
var Colors = class extends import_coc2.BasicList {
  ...
  async loadItems(_context) {
    let {nvim} = this;
    let colors11 = await nvim.eval('split(globpath(&rtp, "colors/*.vim"),"\n")');
    ...
    if (hasPackages) {
      ...
      colors11.push(...packageColors);
    }
    return colors11.map((file) => {
...
var Task = class extends import_events.EventEmitter {
  ...
  start(cmd, args, cwds, patterns) {
    ...
    let filterByName = config.get("filterByName", false);
    for (let cwd of cwds) {
      ...
      rl.on("line", (line) => {
        ...
        if (!filterByName) {
          ...
          let name = import_path3.default.basename(file);
          this.emit("data", {
            label: `${name}	${line}`,
            sortText: file,
            filterText: name,
...
var Task2 = class extends import_events2.EventEmitter {
  ...
  start(cmd, args, cwds, patterns, maxLines) {
    for (let cwd of cwds) {
      let remain = cwds.length;
      ...
      let hasPattern = patterns.length > 0;
      rl.on("line", (line) => {
        if (this.interactive && maxLines > 0 && this.lines >= maxLines)
          return;
        let ms;
        let escaped;
        ...
        if (!ms)
          return;
        let file = ms[1];
        if (!import_path4.default.isAbsolute(file)) {
          file = import_path4.default.join(cwd, ms[1]);
        }
...
Grep source provide some uniformed options to ease differences between rg and ag.`;
  ...
  async loadItems(context) {
    ...
    let maxLines = config.get("maxLines", 0);
    if (cmd == "rg") {
...
var MruList = class extends import_coc8.BasicList {
  ...
  async loadItems(context) {
    ...
    let files = await this.mru.load();
    let config = import_coc8.workspace.getConfiguration("list.source.mru");
    let filterByName = config.get("filterByName", false);
    ...
    return files.map((file, i) => {
      ...
      if (!filterByName) {
        ...
        let name = import_path6.default.basename(file);
        file = findAll ? file : import_path6.default.relative(cwd, file);
        return {
          label: `${name}	${file}`,
          data: {uri},
          sortText: String.fromCharCode(i),
...
var SessionList = class extends import_coc10.BasicList {
  constructor(nvim, extensionPath, saveOnVimLeave) {
    super(nvim);
      ...
      this.disposables.push(import_coc10.workspace.registerAutocmd({
        ...
        callback: async () => {
          ...
          if (!curr) {
            ...
            if (!import_path7.default.isAbsolute(curr))
              return;
            let folder = import_path7.default.dirname(curr);
...
// src/functions.ts
var import_safe10 = __toModule(require_safe());
var Functions = class {
  constructor(nvim) {
    this.nvim = nvim;
    this.name = "functions";
    this.description = "function list";
    this.defaultAction = "open";
    this.actions = [];
    this.actions.push({
      name: "open",
...
")[1]`);
  ...
  async loadItems(_context) {
    ...
    for (let str of list) {
      ...
      res.push({
        label: str.slice(0, 8) + import_safe10.default.magenta(name) + end,
        filterText: name,
        data: {
          funcname: name
        }
...
async function activate(context) {
  ...
  let config = import_coc13.workspace.getConfiguration(void 0, null);
  let disabled = config.get("lists.disabledLists", []);
  ...
  if (!isDisabled("session")) {
    let saveOnVimLeave = config.get("session.saveOnVimLeave", true);
    subscriptions.push(import_coc13.listManager.registerList(new session_default(nvim, context.extensionPath, saveOnVimLeave)));
  }
  ...
  if (!isDisabled("functions")) {
    subscriptions.push(import_coc13.listManager.registerList(new functions_default(nvim)));
  }
}
